...
export default function Results() {
  ...
  return <div className="flex flex-col min-h-full"><ApiNotice errors={apiErrors} />
    ...
    {!activeRace ? <p className="text-center py-10 text-gray-400">No races found</p> : activeRace.status !== "result" ? <div className="flex flex-col items-center py-16 text-gray-500"><AlertCircle size={28}/><p>Race not completed / Results not official</p></div> : <div className="p-3"><div className="bg-white rounded-lg shadow-sm overflow-hidden">{[...activeRace.runners].sort((a,b)=>(a.finishPosition??999)-(b.finishPosition??999)).map((runner)=>{const pickers=picks.filter((p)=>p.runnerId===runner.id).map((p)=>users.find((u)=>u.id===p.userId)?.displayName ?? "?"); const posLabel = runner.finishPosition ?? (runner.finishStatus==="dnf" ? "DNF" : "—"); return <div key={runner.id} className="grid grid-cols-[3rem_1fr_4rem_5rem] items-center px-2 py-2 border-b last:border-0"><div className="text-xs font-bold">{posLabel}</div><div className="flex items-center gap-2"><SilkIcon silkUrl={runner.silkUrl} colors={runner.silkColors} size={20}/><span className="text-xs font-semibold">{runner.horseName}</span></div><div className="text-xs font-bold text-center">{runner.pointsAwarded ?? "—"}</div><div className="text-[10px] text-center">{pickers.join(", ") || "—"}</div></div>;})}</div></div>}
  </div>;
}
